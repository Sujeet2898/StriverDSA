...
public class _4_MColoringProblem {  
    ...
    public static boolean graphColoring(ArrayList<Integer>[] graph, int[] color, int node, int m) {
        int n = graph.length;
        if (solve(node, graph, color, n, m) == true) {
            return true;
        }
        return false;
    }

    private static boolean solve(int node, ArrayList<Integer>[] graph, int[] color, int n, int m) {  // n -> graph size & m -> colour size
        if (node == n) {
            return true;
        }
    ...
    private static boolean isSafe(int node, ArrayList<Integer>[] graph, int[] color, int n, int particularColour) {
        for (int adjacentColour: graph[node]) {
            if (color[adjacentColour] == particularColour) {
                return false;
            }
        }
        return true;
