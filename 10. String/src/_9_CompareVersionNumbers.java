...
public class _9_CompareVersionNumbers {

    // Time Complexity: O(N + M), where M and N are the lengths of the string A and B respectively.
    // Reason: all the characters of both strings will be iterated so the overall time complexity will be   O(N + M)
    // Space Complexity: O(1) --> Since we not using any extra space

    public int compareVersion(String version1, String version2) {

        // Split the string along with dot in an array
        String[] versionArr1 = version1.split("\\.");
        String[] versionArr2 = version2.split("\\.");

        // Ex--> 1.0.0.001 -->split-->[1,0,0,001]-->parseInt-->[1,0,0,1]
        // Now since array has been created so take val1 from arr1 and val2 from arr2
        // Taking parseInt so that the leading zeroes get vanished

        for(int i = 0; i < Math.max(versionArr1.length, versionArr2.length); i++){
            int val1 = (i < versionArr1.length) ? Integer.parseInt(versionArr1[i]) : 0;
            int val2 = (i < versionArr2.length) ? Integer.parseInt(versionArr2[i]) : 0;
            if(val1 < val2) {
                return -1;
            }
            if(val1 > val2) {
